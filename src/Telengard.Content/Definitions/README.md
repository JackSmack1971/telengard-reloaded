...
resolve. Missing optional type directories are allowed so later content slices
can add their files incrementally. The loaded `ContentPack.ContentVersion` is
an explicit input for consumers; it does not change save schema versioning.
