...
def report(a):
    ...
    for f in sorted({str(e.get('skill_fingerprint')) for e in es if e.get('skill_fingerprint')}):
        c=[e for e in es if str(e.get('skill_fingerprint'))==f]; cs=[e for e in c if e.get('event')=='lane_selected']; cr=[e for e in c if e.get('event')=='lane_result']; ca=[e for e in cr if sum(cint(e.get(k)) for k in ('p0','p1','p2'))>0]; cc=[e for e in c if e.get('event')=='command_result']; cf=[e for e in cc if cint(e.get('attempt'))==1]; cfok=[e for e in cf if e.get('result') in {'success','expected_nonzero'}]; cl=[e for e in c if e.get('event')=='lookup_attempt']; clf=[e for e in cl if cint(e.get('attempt'))==1]; clfok=[e for e in clf if e.get('result')=='found']; fps[f]={'runs_started':sum(e.get('event')=='skill_start' for e in c),'runs_ended':sum(e.get('event')=='skill_end' for e in c),'lanes_selected':len(cs),'actionable_lane_yield':pct(len(ca),len(cr)),'lane_escalations':sum(e.get('event')=='lane_escalated' for e in c),'profile_fallbacks':sum(e.get('event')=='lane_profile_fallback' for e in c),'lane_context_missing':sum(e.get('event')=='lane_context_missing' for e in c),'unexpected_lane_context':sum(e.get('event')=='unexpected_lane_context' for e in c),'command_first_attempt_success_rate':pct(len(cfok),len(cf)),'lookup_first_attempt_success_rate':pct(len(clfok),len(clf))}
