...
Read root `AGENTS.md`, the selected TEL ticket, relevant invariants, and only the architecture/spec/code needed for each lane. For TEL-110–TEL-127 or Godot/client work, also read the blueprint/gate documents referenced by the ticket and the active `GODOT-PLAYABLE-VERTICAL-SLICE` ExecPlan.
...
- for TEL-120–TEL-127, synchronization with the active Godot umbrella ExecPlan and relevant gate state;
- no production-art TEL work being marked eligible before `ART-PRODUCTION-READY` has passing evidence.

## Presentation lane

Add this lane whenever Terminal/Godot/UI/presentation behavior or renderer-facing projections/resources change. For Godot/client work read:

- `docs/presentation/GODOT_CLIENT_BLUEPRINT.md`;
- the selected TEL ticket's required blueprint context;
- `docs/gates/GODOT-PLAYABLE-SLICE.md` or `docs/gates/ART-PRODUCTION-READY.md` when the diff claims progress against them.
...
- final production assets being introduced before Art Production Ready passes,
  except explicit concept/style/placeholder work permitted by the blueprint;
