...
For current documentation status, `tasks/README.md` is the TEL-ticket ledger;
`docs/audit-status.json` is the canonical audit-remediation ledger projected into
the remediation playbook and P0 gate by the TEL-119 tooling; and
`BUILD_STATUS.md` is append-only verification history.
...
Read: root `AGENTS.md` → `tasks/index.json` → selected task's context manifest → cited spec sections → relevant invariant/architecture sections → dependency implementations/tests → current `BUILD_STATUS.md`. Use `$telengard-ticket`.
