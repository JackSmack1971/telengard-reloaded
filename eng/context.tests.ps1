...
$full = Invoke-Context @('-Ticket', 'TEL-124')
...
Assert-Condition ($full.Text -match '(?m)^REQUIRED$' -and $full.Text -match 'docs/tasks/TEL-124\.md') 'Full pack omitted required ticket context.'
...
$lane = Invoke-Context @('-Ticket', 'TEL-124', '-Lane', 'presentation')
